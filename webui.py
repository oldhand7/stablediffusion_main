...
def variable_outputs(k):
    k = int(k)
    print("********" , k)
    return [gr.update(visible=True)] * k + [gr.update(visible=False)] * (modules.config.default_max_text_prompt_number - k)
    ...
    while not finished:
        ...
        if len(task.yields) > 0:
            ...
            if flag == 'finish':
                ...
                # delete BTGen temp images, only keep gradio temp images
...
title = f'BTGen {btgen_version.version}'
...
with shared.gradio_root:
    ...
    with gr.Row():
        with gr.Column(scale=2):
            ...
            with gr.Row(elem_classes='type_row'):
                with gr.Column(scale=17):
                    ...
                    text_prompt_number = gr.Number(label='prompt number', minimum=1, maximum=30, value=10)
                    prompt = []
                    for index in range(modules.config.default_max_text_prompt_number):
                        if index < 10:
                            prompt += [gr.Textbox(visible = True, interactive=True,show_label=False, elem_id='positive_prompt',
                                                container=False, autofocus=True, elem_classes='type_row', lines=1)]
                        else: 
                            prompt += [gr.Textbox(visible = False, interactive=True,show_label=False, elem_id='positive_prompt',
                                                container=False, autofocus=True, elem_classes='type_row', lines=1)]
                    # generate_button.click(lambda: ([gr.update(visible=True, interactive=True)] * 10), outputs=text_prompt)
                    text_prompt_number.change(variable_outputs, text_prompt_number, prompt)
        ...
        ctrls += [style_selections,performance_selection, aspect_ratios_selection, image_number, output_format, image_seed, sharpness, guidance_scale]
        ...
        def trigger_describe(mode, img):
            ...
            if mode == flags.desc_type_anime:
                ...
                return default_interrogator_anime(img), ["Fooocus V2", "BTGen Masterpiece"]
